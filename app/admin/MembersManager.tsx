  ...
  return (
    <div className="max-w-5xl mx-auto p-6 text-white">
      ...
      <div className="bg-white/5 border border-white/10 rounded-lg p-4 mb-8 flex flex-wrap gap-3 items-end">
        ...
        <button onClick={addMember} className="cursor-pointer bg-[#2683EB] hover:bg-[#1f6fd0] rounded px-4 py-2 font-medium">
          Add member
        </button>
      </div>
        ...
        Object.entries(groups).map(([netLabel, list]) => (
          <div key={netLabel} className="mb-8">
            <h3 className="text-lg font-medium text-[#2683EB] mb-2">
              {netLabel}
              {!filterChapter && (
                <span className="text-white/40 text-sm ml-2">({chapterName(list[0].chapter_id)})</span>
              )}
            </h3>
                        ...
                        <>
                          ...
                          <td className="py-2 whitespace-nowrap">
                            <button onClick={() => saveEdit(m.member_id)}
                              className="cursor-pointer text-green-400 hover:text-green-300 text-sm mr-3">Save</button>
                            <button onClick={cancelEdit}
                              className="cursor-pointer text-white/50 hover:text-white/80 text-sm">Cancel</button>
                          </td>
                        </>
                      ) : (
                        <>
                          ...
                          <td className="py-2 whitespace-nowrap">
                            <button onClick={() => startEdit(m)}
                              className="cursor-pointer text-[#2683EB] hover:text-blue-300 text-sm mr-3">Edit</button>
